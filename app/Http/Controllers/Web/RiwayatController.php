...
{
    return $this->detailAdmin2($id); // bisa pakai detailKasir yang sudah ada
}
...
public function addLayananAdmin2(Request $request, $id)
{
    $request->validate([
        'id_layanan' => 'required|exists:layanan,id_layanan',
        'id_jenis_layanan' => 'required|exists:jenis_layanan,id_jenis_layanan', // ✅ TAMBAH INI
        'qty'        => 'required|numeric|min:0.01',
        'parfum'     => 'nullable|exists:parfum,id_parfum',
    ]);

    $jenis = JenisLayanan::findOrFail($request->id_jenis_layanan); // ✅ AMBIL LANGSUNG
