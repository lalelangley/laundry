{{-- FE-DOC: Template frontend untuk resources/views/admin/layanan_edit.blade.php. Tambahan komentar di file ini dipakai sebagai penjelas struktur Blade, Tailwind, CSS, dan JavaScript tanpa mengubah behavior. --}}
...
<div class="px-6 mt-8 w-full max-w-full pb-8">
    ...
    {{-- SUCCESS --}}
    @if(session('success'))
        <div id="successMsg" class="bg-green-500 text-white p-3 rounded-xl mb-5 text-base shadow-md flex items-center gap-2">
    ...
    <form action="{{ route('layanan.update', $layanan->id_layanan) }}" method="POST" class="bg-white rounded-2xl shadow-lg p-6">
        ...
        <div class="mb-6">
            ...
            <input type="text"
                ...
                class="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-base focus:ring-2 focus:ring-yellow-400 focus:border-yellow-400 outline-none transition-all"
        ...
        @php
            ...
            if (old('proses')) $selectedProses = old('proses');
        @endphp
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-3 gap-4 w-full">
                @foreach ($prosesList as $p)
                    ...
                    @endphp
                    <label class="proses-label flex items-center gap-2 px-4 py-3 border-2 rounded-xl cursor-pointer text-base font-medium transition-all
            ...
            <p class="text-xs text-gray-500 mt-2">
                <i class="bi bi-info-circle"></i> Pilih minimal 1 proses layanan
            </p>
        </div>
        ...
        <div class="border-t-2 border-gray-100 pt-6">
                ...
                @foreach ($layanan->jenis as $jenis)
                    <a href="{{ route('layanan.jenis.edit', $jenis->id_jenis_layanan) }}?from={{ $layanan->id_layanan }}"
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-green-600 font-semibold text-base">Rp {{ number_format($jenis->harga, 0, ',', '.') }} / {{ $jenis->satuan->nama_satuan ?? '-' }}</p>
            ...
            {{-- TAMBAH JENIS --}}
            <a href="{{ route('layanan.jenis.session.form', $layanan->id_layanan) }}"
                class="flex items-center justify-center gap-2 mt-6 bg-yellow-400 hover:bg-yellow-500 active:scale-95 transition-all text-black text-center py-4 rounded-xl font-bold text-lg shadow-md hover:shadow-lg">
        ...
        <div class="flex gap-3 justify-end mt-8 pt-6 border-t-2 border-gray-100">
            <a href="{{ route('layanan.index') }}"
                class="bg-gray-200 hover:bg-gray-300 active:scale-95 transition-all px-8 py-3 rounded-xl font-bold text-gray-700 text-lg shadow-md inline-flex items-center gap-2">
                <i class="bi bi-x-circle"></i>
                <span>Batal</span>
            </a>
            <button type="submit"
                class="bg-green-600 hover:bg-green-700 active:scale-95 transition-all px-8 py-3 rounded-xl font-bold text-white text-lg shadow-lg hover:shadow-xl inline-flex items-center gap-2">
...
@section('scripts')
<script src="{{ asset('js/admin/layanan-edit.js') }}"></script>
@endsection