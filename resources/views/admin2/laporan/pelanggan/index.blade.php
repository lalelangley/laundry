{{-- FE-DOC: Template frontend untuk resources/views/admin2/laporan/pelanggan/index.blade.php. Tambahan komentar di file ini dipakai sebagai penjelas struktur Blade, Tailwind, CSS, dan JavaScript tanpa mengubah behavior. --}}
...
<div class="bg-yellow-400 px-5 py-4 rounded-b-3xl sticky top-0 z-20 shadow">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-2">
            <a href="{{ route('admin2.laporan.pelanggan.export') }}?{{ http_build_query(array_merge(request()->all(), ['format' => 'excel'])) }}"
            ...
            <a href="{{ route('admin2.laporan.pelanggan.export') }}?{{ http_build_query(array_merge(request()->all(), ['format' => 'pdf'])) }}"
               class="flex items-center gap-1.5 text-sm font-semibold bg-white bg-opacity-30 hover:bg-opacity-50 px-3 py-2 rounded-full transition-all">
...
<form method="GET" id="filterForm" class="px-6 mt-6 space-y-3">
    ...
    <div class="bg-white rounded-3xl sm:rounded-full shadow flex flex-col sm:flex-row items-stretch sm:items-center px-4 py-3 gap-3">
        ...
        @if(request()->hasAny(['q', 'dari', 'sampai', 'sort']))
            <a href="{{ route('admin2.laporan.pelanggan.index') }}" class="bg-gray-200 hover:bg-gray-300 px-5 py-2 rounded-full font-bold transition-all text-center w-full sm:w-auto">
                Reset
            </a>
        @endif
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    resetBtn.addEventListener('click', function() {
        window.location.href = '{{ route("admin2.laporan.pelanggan.index") }}';
    });
