...
{{-- FE-DOC: Header halaman dipakai untuk judul modul, navigasi balik, dan kadang tombol export cepat. --}}
    <div class="bg-yellow-400 px-5 py-5 md:px-8 md:py-6 rounded-b-3xl flex items-center gap-3 md:gap-4 shadow-lg">
        <a href="{{ route('admin2.dashboard') }}" class="text-black text-2xl md:text-3xl font-bold hover:opacity-80 transition">
            <i class="bi bi-arrow-left"></i>
        </a>
        <span class="text-xl md:text-2xl font-bold text-gray-900">Kelola Laporan</span>
    </div>
...
<div id="exportModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center px-4 z-50 hidden">
    ...
    <div class="bg-[#fcfcfb] rounded-[32px] shadow-2xl max-w-3xl w-full max-h-[95vh] overflow-hidden animate-fadeIn export-modal-shell">
        <div class="overflow-y-auto max-h-[95vh]">
            <form action="{{ route('admin2.laporan.transaksi.export') }}" method="POST" class="p-5 md:p-10 space-y-6" data-no-loading>
                @csrf
...
function openTab(tabName) {
    ...
    document.getElementById('tab-laporan').className = defaultClass;
    document.getElementById('tab-transaksi').className = defaultClass;
    document.getElementById('tab-data').className = defaultClass;
