...
@php
    ...
    if ($from === 'pesanan_online') {
        $backUrl = route('admin2.pesanan.online.index');
    } elseif ($riwayat) {
        $backUrl = route('admin2.riwayat.detail', $riwayat->id_transaksi);
    } else {
...
<div class="min-h-screen bg-gray-50 pb-32">
    ...
    <div class="mx-6 mt-6">
        <div class="bg-white rounded-3xl shadow-xl p-5">
            ...
            {{-- LIST LAYANAN --}}
            @if($riwayat)
                <form action="{{ route('admin2.riwayat.update', $riwayat->id_transaksi) }}" method="POST" id="formUpdate">
                    @csrf
                    @method('PUT')
...
btnConfirmDelete.onclick = function() {
    ...
    // Send delete request
    fetch(`/admin2/riwayat/detail/${deleteDetailId}`, {
        method: 'DELETE',
        headers: {
