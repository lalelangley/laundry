{{-- FE-DOC: Template frontend untuk resources/views/admin2/layanan/layanan_create.blade.php. Tambahan komentar di file ini dipakai sebagai penjelas struktur Blade, Tailwind, CSS, dan JavaScript tanpa mengubah behavior. --}}
...
<div class="bg-yellow-400 px-5 py-4 rounded-b-3xl flex items-center gap-3 shadow">
    <a href="{{ route('admin2.layanan.index') }}" class="text-black text-3xl font-bold">←</a>
...
<div class="px-5 mt-6">
    ...
    <a href="{{ route('admin2.layanan.jenis.create', ['id_layanan' => $layanan_id]) }}" 
    ...
    {{-- FORM CREATE LAYANAN      --}}
    {{-- ======================== --}}
    <form action="{{ route('admin2.layanan.store') }}" method="POST" class="bg-white p-5 rounded-2xl shadow">
        @csrf
            ...
            <div class="grid grid-cols-3 gap-3 mt-2">
                @foreach ($prosesList as $p)
                    <label class="flex items-center gap-2 p-3 border rounded-xl cursor-pointer hover:bg-gray-100">
                        <input type="checkbox" name="proses[]" value="{{ $p }}" class="w-5 h-5">
                        <span>{{ $p }}</span>
                    </label>
                @endforeach
            </div>
