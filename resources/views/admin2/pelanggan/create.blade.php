{{-- FE-DOC: Template frontend untuk resources/views/admin2/pelanggan/create.blade.php. Tambahan komentar di file ini dipakai sebagai penjelas struktur Blade, Tailwind, CSS, dan JavaScript tanpa mengubah behavior. --}}
...
<div class="px-5 py-6">
   ...
   <form action="{{ request('from') === 'transaksi' ? route('admin2.transaksi.pelanggan.store') : route('admin2.pelanggan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf
        ...
        <div class="bg-white rounded-2xl shadow-md p-5 space-y-5 border border-yellow-300">
            ...
            <div>
                ...
                <div class="flex items-center gap-8 mt-2">
                    ...
                    <input type="radio" name="jk" value="L" class="accent-yellow-500" {{ old('jk', 'L') === 'L' ? 'checked' : '' }}>
                    <span>Pria</span>
                </label>

                <label class="flex items-center gap-2">
                    <input type="radio" name="jk" value="P" class="accent-yellow-500" {{ old('jk') === 'P' ? 'checked' : '' }}>
                    <span>Wanita</span>
                </label>
                </div>
                @error('jk')
