        ...
        <div id="parfumList" class="space-y-4">
            ...
            <div class="parfum-item bg-white p-5 rounded-2xl shadow-lg hover:shadow-xl transition-all hover:-translate-y-1 flex items-center gap-4"
                ...
                <div class="flex items-center gap-2">
                    ...
                    <form action="{{ route('parfum.destroy', $p->id_parfum) }}" method="POST" class="inline delete-form">
                        @csrf
                        @method('DELETE')
                        <button type="button"
...
function confirmDelete(button) {
    const namaParfum = button.getAttribute('data-nama');
    const form = button.closest('form');
    
    Swal.fire({
        title: 'Hapus Parfum?',
        html: `Apakah Anda yakin ingin menghapus<br><strong class="text-pink-600">${namaParfum}</strong>?`,
        icon: 'warning',
