...
{{-- HEADER --}}
{{-- FE-DOC: Header halaman dipakai untuk judul modul, navigasi balik, dan tombol export cepat. --}}
<div class="bg-yellow-400 px-5 py-4 rounded-b-3xl sticky top-0 z-20 shadow">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-2">
            <a href="{{ route('laporan.pelanggan.export') }}?{{ http_build_query(array_merge(request()->all(), ['format' => 'excel'])) }}"
    ...
    <div class="bg-white rounded-xl p-4 shadow border-2 border-green-400">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <div class="text-base font-bold text-green-600 break-words">
    ...
    <div class="bg-white rounded-xl overflow-hidden shadow">
        <div class="overflow-x-auto">
            <div class="grid min-w-[860px] grid-cols-[56px_2fr_1.2fr_1fr_1.2fr] bg-yellow-400 text-center font-bold text-sm py-3 border-b border-yellow-500">
                ...
                <div class="flex items-center justify-center gap-1">
                    <i class="bi bi-cash"></i> Total Belanja
                </div>
            </div>
            ...
            @forelse ($data as $i => $item)
                <div class="grid min-w-[860px] grid-cols-[56px_2fr_1.2fr_1fr_1.2fr] text-center py-4 border-b border-gray-100 hover:bg-yellow-50 transition-all text-sm">
                    ...
                    <div class="text-gray-600 font-semibold flex items-center justify-center px-2 break-all">
                        {{ $item->no_hp ?: '-' }}
                    </div>
                    <div class="font-semibold text-gray-800 flex items-center justify-center px-2">
                        {{ $item->total_transaksi }}
                    </div>
                    <div class="font-bold text-orange-600 flex items-center justify-center px-2 break-words">
    ...
    <div class="px-5 mt-6 flex items-center justify-center gap-2">
        ...
        <div class="flex items-center gap-2">
            ...
            @foreach ($data->getUrlRange(max(1, $data->currentPage()-2), min($data->lastPage(), $data->currentPage()+2)) as $page => $url)
                <a href="{{ $url }}"
                   class="w-9 h-9 flex items-center justify-center rounded-full text-sm font-bold transition-all {{ $page == $data->currentPage() ? 'bg-yellow-400 text-gray-900 shadow' : 'bg-white hover:bg-yellow-50 text-gray-600 shadow-sm' }}">
                    {{ $page }}
                </a>
            @endforeach
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    resetBtn.addEventListener('click', function() {
        window.location.href = '{{ route("laporan.pelanggan.index") }}';
    });
