{{-- FE-DOC: Template frontend untuk resources/views/laporan/bayar/index.blade.php. Tambahan komentar di file ini dipakai sebagai penjelas struktur Blade, Tailwind, CSS, dan JavaScript tanpa mengubah behavior. --}}
...
<div class="bg-yellow-400 px-5 py-4 rounded-b-3xl sticky top-0 z-20 shadow">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-2">
            <a href="{{ route('laporan.bayar.export', ['dari' => $tglAwal, 'sampai' => $tglAkhir]) }}"
            ...
            <a href="{{ route('laporan.bayar.export', ['dari' => $tglAwal, 'sampai' => $tglAkhir, 'format' => 'pdf']) }}"
               class="flex items-center gap-1.5 text-sm font-semibold bg-white bg-opacity-30 hover:bg-opacity-50 px-3 py-2 rounded-full transition-all">
...
<div class="px-6 mt-5 grid grid-cols-1 sm:grid-cols-3 gap-4">
    <div class="bg-white rounded-xl p-4 shadow border-2 border-yellow-400">
        <div class="flex items-center gap-3">
            ...
            <div>
                <div class="text-xs text-gray-500 font-semibold">Total Penggunaan</div>
                <div class="text-2xl font-bold text-gray-800">{{ $totalPenggunaan }}</div>
            </div>
        </div>
    </div>
    <div class="bg-white rounded-xl p-4 shadow border-2 border-orange-400">
        <div class="flex items-center gap-3">
            <div class="w-11 h-11 bg-orange-100 rounded-full flex items-center justify-center shrink-0">
                <i class="bi bi-credit-card-2-front text-orange-600 text-xl"></i>
            </div>
            <div>
                <div class="text-xs text-gray-500 font-semibold">Jumlah Metode</div>
    ...
    <div class="bg-white rounded-xl overflow-hidden shadow">
        <div class="overflow-x-auto">
            <div class="grid min-w-[640px] grid-cols-[56px_2fr_1fr] bg-yellow-400 text-center font-bold text-sm py-3 border-b border-yellow-500">
                <div>#</div>
                <div class="flex items-center justify-center gap-1"><i class="bi bi-credit-card"></i> Metode Bayar</div>
                <div class="flex items-center justify-center gap-1"><i class="bi bi-bar-chart"></i> Total Penggunaan</div>
            </div>
            ...
            @forelse ($data as $index => $item)
                <div class="grid min-w-[640px] grid-cols-[56px_2fr_1fr] text-center py-4 border-b border-gray-100 hover:bg-yellow-50 transition-all text-sm">
                    ...
                    <div class="font-semibold text-gray-800 px-3 text-left flex items-center">{{ $item->nama_metode_bayar }}</div>
                    <div class="font-bold text-orange-600 flex items-center justify-center px-2">{{ $item->total_penggunaan }}</div>
                </div>
            @empty
                <div class="text-center py-20 text-gray-400">
                    <i class="bi bi-inbox text-6xl mb-3 block text-gray-300"></i>
                    <p class="font-semibold text-lg">Tidak ada data metode bayar</p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    resetBtn.addEventListener('click', function() {
        window.location.href = '{{ route("laporan.bayar.index") }}';
    });
