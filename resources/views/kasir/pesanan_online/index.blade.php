    ...
    <div class="px-8 py-6">
        ...
        <div class="mb-6 bg-white p-2 rounded-2xl shadow-lg overflow-x-auto">
            <div class="grid grid-cols-6 gap-2">
                @php
                    $tabs = [
                        'menunggu_konfirmasi' => ['label' => 'Menunggu', 'icon' => 'hourglass-split'],
                        'dikonfirmasi' => ['label' => 'Dikonfirmasi', 'icon' => 'check-square'],
                        'proses' => ['label' => 'Proses', 'icon' => 'arrow-repeat'],
                        'siap_di_ambil' => ['label' => 'Siap Diambil', 'icon' => 'box-seam'],
                        'selesai' => ['label' => 'Selesai', 'icon' => 'check-all'],
                        'ditolak' => ['label' => 'Ditolak', 'icon' => 'x-circle']
                    ];
                @endphp
                ...
                @foreach ($tabs as $key => $data)
                    <a href="{{ route('kasir.pesanan.online.index', ['tab'=>$key]) }}"
                       ...
                       {{ $tab == $key ? 'bg-yellow-400 text-black shadow-md scale-105' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5" id="pesananList">
            @forelse ($pesanan as $p)
                <div class="group relative" data-nama="{{ strtolower($p->nama_pelanggan ?? '') }}" data-id="{{ $p->id_transaksi }}">
                    <a href="{{ route('kasir.pesanan.online.detail', $p->id_transaksi) }}" class="block">
                        <div class="relative bg-white shadow-lg rounded-2xl p-6 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border-l-4
                            ...
                            <div class="flex justify-between items-start mb-4 pb-4 border-b-2 border-gray-100">
                                <div class="flex-1">
                                    <h2 class="font-bold text-xl text-gray-800 mb-1">{{ $p->nama_pelanggan ?? 'Guest' }}</h2>
                                ...
                                {{-- DELETE BUTTON --}}
                                <form action="{{ route('kasir.pesanan.online.destroy', $p->id_transaksi) }}"
                                      method="POST"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                            class="bg-red-500 text-white p-2 rounded-lg shadow hover:bg-red-600 hover:scale-110 transition-all"
                            ...
                            {{-- PRICE TAG --}}
                            <div class="bg-gradient-to-r from-yellow-400 to-yellow-500 text-gray-900 px-4 py-3 rounded-xl mb-4 shadow-md">
                                <p class="text-sm font-semibold mb-1">Total Harga</p>
                                <p class="text-2xl font-bold">Rp {{ number_format($p->total_harga,0,',','.') }}</p>
                            </div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex items-center gap-3 text-sm">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="font-semibold text-gray-800">{{ $p->detail_transaksi->sum('qty') }} item</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-4 pt-4 border-t-2 border-gray-100 flex flex-wrap gap-2">
                                <span class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-white text-xs font-semibold
                                ...
                                @if ($p->status_bayar == 'belum_bayar' || $p->status_bayar == 'belum_lunas')
                                    <span class="inline-flex items-center gap-1 px-3 py-1.5 bg-red-500 text-white rounded-lg text-xs font-semibold">
                                        <i class="bi bi-x-circle"></i>
                                        Belum Bayar
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs font-semibold">
                            ...
                            <div class="mt-4 flex flex-wrap gap-2">
                                @if($p->status_transaksi == 'menunggu_konfirmasi')
                                    <a href="{{ route('kasir.pesanan.online.terima', $p->id_transaksi) }}"
                                       onclick="event.preventDefault(); if(confirm('Terima pesanan ini?')) window.location.href=this.href;"
                                       class="px-4 py-2 bg-green-500 text-white rounded-lg shadow hover:bg-green-600">
                                        <i class="bi bi-check-lg"></i> Terima
                                    </a>
                                    <a href="{{ route('kasir.pesanan.online.tolak', $p->id_transaksi) }}"
                                       onclick="event.preventDefault(); if(confirm('Tolak pesanan ini?')) window.location.href=this.href;"
                                ...
                                @if($p->status_transaksi == 'dikonfirmasi')
                                    <a href="{{ route('kasir.pesanan.online.proses', $p->id_transaksi) }}"
                                       class="px-4 py-2 bg-blue-500 text-white rounded-lg shadow hover:bg-blue-600">Proses</a>
                                @endif

                                @if($p->status_transaksi == 'proses')
                                    <a href="{{ route('kasir.pesanan.online.siap_di_ambil', $p->id_transaksi) }}"
                                       class="px-4 py-2 bg-teal-500 text-white rounded-lg shadow hover:bg-teal-600">Siap Diambil</a>
                                @endif

                                @if($p->status_transaksi == 'siap_di_ambil')
                                    <a href="{{ route('kasir.pesanan.online.selesai', $p->id_transaksi) }}"
                                       class="px-4 py-2 bg-green-500 text-white rounded-lg shadow hover:bg-green-600">Selesai</a>
                                @endif
                            </div>
