            ...
            <div class="bg-white rounded-xl shadow p-5 space-y-3">
                ...
                @elseif($pesanan->status_transaksi === 'siap_di_antar')
                  ...
                  {{-- ✅ BUTTON PEMBAYARAN CASH (UNTUK DELIVERY DELIVERED) --}}
                    ...
                    <a href="{{ route('kasir.pesanan.online.bukti-pembayaran', $pesanan->id_transaksi) }}?from_tab={{ request()->get('from_tab', 'pickup') }}"
                ...
                @else
                   ...
                   {{-- BUKTI PEMBAYARAN (TRANSFER) --}}
                    ...
                    {{-- PROSES --}}
                    @if($dataLengkap && in_array($pesanan->status_transaksi, ['antrian', 'menunggu_konfirmasi', 'dikonfirmasi']))
                    <a href="{{ route('kasir.pesanan.online.proses',$pesanan->id_transaksi) }}?from_tab={{ request()->get('from_tab', 'pickup') }}"
...
@if($pesanan->status_transaksi == 'proses')
        ...
        {{-- NORMAL: PILIHAN PICKUP / DELIVERY --}}
            <div class="bg-blue-50 border border-blue-200 p-3 rounded-lg">
                ...
                <div class="grid grid-cols-2 gap-2">
                   <a href="{{ route('kasir.pesanan.online.siap_di_ambil', $pesanan->id_transaksi) }}?from_tab={{ request()->get('from_tab', 'pickup') }}"
    ...
    @endif
                    ...
                    <a href="{{ route('kasir.pesanan.online.selesai',$pesanan->id_transaksi) }}?from_tab={{ request()->get('from_tab', 'pickup') }}"
                    class="block text-center py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 flex items-center justify-center gap-2">
                        <i class="bi bi-check-circle"></i>
                        <span>Selesaikan Pesanan</span>
                    </a>
...
<div id="isiDataModal"
    ...
    <div class="bg-white rounded-xl max-w-md w-full p-6 shadow-2xl max-h-[90vh] overflow-y-auto"
        ...
        <form action="{{ route('kasir.pesanan.online.updateData', $pesanan->id_transaksi) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
...
<div id="deliveryModal"
    ...
    <div class="bg-white rounded-xl max-w-md w-full p-6 shadow-2xl"
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('kasir.pesanan.online.list-driver', $pesanan->id_transaksi) }}?from_tab={{ request()->get('from_tab', 'pickup') }}"
