...
<div class="bg-yellow-400 px-5 py-5 rounded-b-[32px] flex items-center gap-3 shadow-lg">
    <a href="{{ route('kasir.pengaturan.index') }}" class="text-black text-3xl font-bold">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    btnTambah.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch("{{ route('kasir.pengaturan.metode.store') }}", {
                method: "POST",
                headers: {
    ...
    btnKonfirmasiHapus.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch(`/kasir/pengaturan/metode-bayar/${id}`, {
                method: 'DELETE',
                headers: { 
                    'X-CSRF-TOKEN': "{{ csrf_token() }}",
                    'Content-Type': 'application/json'
