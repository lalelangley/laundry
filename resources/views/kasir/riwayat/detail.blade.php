        ...
        <div class="grid grid-cols-12 gap-6">
                ...
                <div class="bg-white shadow-sm rounded-2xl p-6 border border-gray-200 hover:shadow-md transition-all">
                    ...
                    <div class="flex justify-between items-center mb-5 pb-4 border-b border-gray-200">
                        ...
                        @if($transaksi->status_transaksi == 'antrian')
                            <a href="{{ route('kasir.riwayat.edit', $transaksi->id_transaksi) }}"
            ...
            <div class="col-span-12 lg:col-span-4 space-y-6">
                ...
                <div class="space-y-3">
                    {{-- Tombol Proses --}}
                    @if($transaksi->status_transaksi == 'antrian')
                        <a href="{{ route('kasir.riwayat.proses', $transaksi->id_transaksi) }}" 
                    ...
                    <form id="formBatal" action="{{ route('kasir.riwayat.batal', $transaksi->id_transaksi) }}"
                        method="GET">
            ...
            <form id="formBayar" class="space-y-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-2">
                        ...
                        Isi sesuai sisa bayar untuk melunasi transaksi
                    </p>
                </div>
...
document.getElementById('formBayar').addEventListener('submit', function(e) {
    ...
    // ✅ VALIDASI DIPERBAIKI - Boleh kosong (0)
    // Hanya validasi jika jumlah bayar negatif
    if (jumlahBayar < 0) {
        Swal.fire({
            icon: 'error',
    ...
    // ✅ Validasi jika melebihi sisa bayar
    ...
    // ✅ Proses submit jika validasi lolos
    prosesSubmitPembayaran(jumlahBayar);
});

// ✅ Fungsi terpisah untuk submit pembayaran
function prosesSubmitPembayaran(jumlahBayar) {
    ...
    fetch("{{ route('kasir.riwayat.bayar.submit', $transaksi->id_transaksi) }}", {
        method: 'POST',
        body: formData,
        headers: {
    ...
    .then(data => {
        if (data.success) {
            // Update data lokal
            dpTerbayar += jumlahBayar;
            sisaBayar = totalTagihan - dpTerbayar;
            
            // Update status pembayaran
            if (sisaBayar <= 0) {
                currentStatusBayar = 'lunas';
            } else if (dpTerbayar > 0) {
                currentStatusBayar = 'DP';
            } else {
                currentStatusBayar = 'belum bayar';
            }
            ...
            // ✅ Show success message - disesuaikan untuk DP = 0
            if (jumlahBayar === 0) {
                Swal.fire({
                    icon: 'info',
                ...
                Swal.fire({
                    ...
                    // Jika lunas, reload halaman untuk update semua data
                    if (sisaBayar <= 0) {
                        location.reload();
                    }
                });
...
function confirmBatal() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            window.location.href = document.getElementById('formBatal').action;
        }
    });
}
